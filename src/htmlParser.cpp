...
public:
    ...
    {
        LinkedList<html_node> tag_list;
        ds::stack<html_node *> nodestack;
        size_t i = 0;
        ...
        while (i < htmlcontent.size())
        {
            ...
            else if (htmlcontent[i] == '<')
            {
                ...
                {
                    if (nodestack.getTop()->tag_name == tagname.substr(1))
                    {
                        nodestack.pop();
                    }
                    else
                ...
                {    // 创建新节点并设置其标签名和完整的标签内容
                    ...
                    {
                        if (is_SpecialBlockTag(nodestack.getTop()->tag_name) && !isInlineTag(tagname)) // 特殊块级元素（<h1>-<h6>、<p>、<dt>）：只能嵌套⾏内元素
                        {
                            cout << "Debug: stack top tag name = [" << nodestack.getTop()->tag_name << "]" << endl;
                        ...
                        {
                            if (isInlineTag(nodestack.getTop()->tag_name) && nodestack.getTop()->tag_name != "a") // 除了<a>,块级元素不能嵌套在行内元素中。
                            {
                                cout << "Debug: stack top tag name = [" << nodestack.getTop()->tag_name << "]" << endl;
                                cout << "Debug: now_tag = [" << tagname << "]" << endl;
                        ...
                        else if (tagname == "a" && nodestack.getTop()->tag_name == "a") // 特殊⾏内元素（<a>）：可以嵌套块级和⾏内元素，但不能嵌套⾃身。
                        {
                        ...
                        nodestack.getTop()->list.append(new_node); // 合法标签作为当前节点的子节点
                        nodestack.push(new_node);
                    }
            ...
            else
            {
                ...
                if (!text.empty() && !nodestack.isEmpty())
                {
                    ...
                    nodestack.getTop()->list.append(textnode);
                }
    ...
    void text(string address) // 返回对应地址的所有文本
    {
        ...
        string result;
        tree_travel(root, addresses.getHead(), result, "text");
    ...
    void outerHtml(string address) // 返回对应地址的所有html代码
    {
        ...
        string result;
        tree_travel(root, addresses.getHead(), result, "html");
    ...
    void tree_travel(html_node *Hnode, Node<string> *tag_name, std::string &result, string sign) // 按照地址遍历
    {
        ...
        Node<html_node *> *L = Hnode->list.getHead();
        if (tag_name->next == nullptr)
        {
            while (L != nullptr)
            {
                Node<html_node *> *nownode = L;
                if (nownode->val->tag_name == tag_name->val)
                {
                    if (sign == "text")
                    {
                        std::string textcontent = extract_text(nownode->val, nownode->val->tag_name);
                    ...
                    {
                        std::string textcontent = extract_html(nownode->val, nownode->val->tag_name);
                        if (!textcontent.empty())
                        {
                            if (!result.empty() && result.back() != '\n')
                            {
        ...
        while (L != nullptr)
        {
            ...
            if (nownode->val->tag_name == tag_name->val)
            {
                tree_travel(nownode->val, tag_name->next, result, sign);
            }
            L = nownode->next;
        }
    }
    string extract_html(html_node *node, string parent_tag) // 提取标签下所有的html代码
    {
        ...
        // 遍历子节点并收集所有文本内容
        Node<html_node *> *child = node->list.getHead();
        while (child != nullptr)
        {
            text += extract_html(child->val, node->tag_name);
            text += ("</" + node->tag_name + ">");
            child = child->next;
        }
    ...
    string extract_text(html_node *node, string parent_tag) // 提取表签下所有的文本
    {
        ...
        while (child != nullptr)
        {
            text += extract_text(child->val, node->tag_name);
            child = child->next;
        }
        return trim_and_add_spaces(text);
    }
    ...
    void printTree(html_node *node, int depth = 0) // 打印某个节点下的整个html文件
    {
        ...
        while (child != nullptr)
        {
            printTree(child->val, depth + 1); // 深度加一，表示子节点
            child = child->next;
        }
